...
9. 상품명은 확실할 때만 쓰고, 불확실하면 '지금 보시는 상품'이라고 말한다.
"""
...
def is_product_page(url: str, pn: str) -> bool:
    url = (url or "").lower()
    pn = (pn or "").strip()
    return ("product/detail" in url) or ("product_no=" in url) or bool(pn)
...
def recommend_size(height_cm, weight_kg, top_size, product_category, size_options):
    ...
    if weight is None:
        if top_size and top_size.strip():
            return {
                ...
                "reason": "입력하신 평소 사이즈를 기준으로 먼저 보시는 게 가장 안전합니다."
            }
        return {
    ...
    if contains_alpha_sizes(options):
        return {
            ...
            "reason": "현재 체중 기준으로 가장 무난한 옵션입니다."
        }

    if contains_korean_sizes(options):
        return {
            "recommended": pick_from_korean(weight, options),
            "reason": "현재 체중 기준으로 가장 가까운 옵션입니다."
        }

    return {
        "recommended": options[0],
        "reason": "첫 번째 유효 옵션을 기준으로 추천드렸습니다."
    }
...
def build_assistant_system_message(app_ctx: dict, user_input: str) -> str:
    ...
    if is_coordi_request(user_input) and product_ctx:
        ...
        if coordi_items:
            extra_rules.append(
                "코디 추천 요청에는 한 상품만 말하지 말고, 아래 후보 중 2~3개를 추천하세요: " + ", ".join(coordi_items)
            )
    ...
    if purchase_personalization.get("is_logged_in") and purchase_personalization.get("summary"):
        extra_rules.append(
            f"로그인 고객 참고 정보: {purchase_personalization['summary']}. 화면에 과하게 드러내지 말고 추천 정확도에만 활용하세요."
        )
...
if app_ctx["mode"] == "product":
    if app_ctx["current_product"]:
        product_label = app_ctx["current_product"]["product_name"]
    else:
        product_label = "지금 보시는 상품"
else:
    product_label = "일반 상담"
...
if not st.session_state.messages:
    if st.session_state.mode == "product":
        ...
        welcome = (
            ...
            "지금은 일반 상담 상태예요.\n"
            "상품별 정확한 상담은\n"
            "상품 상세페이지에서 채팅 버튼을 다시 눌러주시면\n"
            "그 상품 기준으로 더 정확하게 도와드릴 수 있어요 :)"
        )
