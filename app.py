...
def get_coordi_candidates(product_ctx: dict):
    ...
    return COORDI_DB.get(category, COORDI_DB["기타"])[:3]
...
def build_assistant_system_message(app_ctx: dict, user_input: str) -> str:
    ...
    if product_ctx:
        ...
        extra_rules.append(
            "현재는 일반 상담 상태입니다. 특정 상품 상담이 필요한 경우 상품 상세페이지에서 채팅 버튼을 다시 누르면 더 정확하다고 자연스럽게 안내하세요."
        )
    ...
    if is_coordi_request(user_input) and product_ctx:
        ...
        if coordi_items:
            ...
            extra_rules.append("각 추천 상품마다 길지 않게 한 줄 이유만 덧붙이세요.")
    ...
    if purchase_personalization.get("is_logged_in"):
        ...
        if is_purchase_history_request(user_input):
            extra_rules.append(
                "고객이 이전 구매 경험을 묻는 경우, 최근 구매 사이즈나 상품을 참고한 듯 자연스럽고 짧게 답하세요."
            )
